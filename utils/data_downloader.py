...
def make_csv(data_name):
  ...
  csv_path = os.path.join(data_folder, data_folder.split(os.path.sep)[-1] + '.csv')

  down_func = down_funcs[data_name]
  down_func(data_folder, csv_path)
...
def download_volatility(data_folder, output_file):
...
def download_electricity(data_folder, output_file):
...
def download_traffic(data_folder, output_file):
...
def process_favorita(data_folder, output_file):
